#!/usr/bin/env python3
"""Release-controlled, staging-only installation of one immutable public-film pack.
...
MANIFEST_RELATIVE = "data/alpha_public_films_12s_v1.json"
POLICY_RELATIVE = "capabilities/staging-public-films.json"
OVERRIDE_RELATIVE = "compose.staging-public-films.yaml"
APPLICATION_ROOT = Path("/srv/authority-closers/application")
RELEASES_ROOT = APPLICATION_ROOT / "releases"
MEDIA_ROOT = APPLICATION_ROOT / "media-staging"
ROOT_UID = 0
ROOT_GID = 0
FILE_COUNT = 30
...
def _fail(message: str) -> PackError:
    return PackError(f"staging public-film pack refused: {message}")
...
def load_policy(release: Path, environment: str) -> bool:
    # A staging policy never grants production or worker/migrator capabilities.
    ...
    try:
        ...
        return False  # Immutable releases predating this capability stay off.
    ...
    if (
        ...
        or (
            value["schema_version"] != "ac-staging-public-films-deployment.v1"
...
def load_manifest(release: Path) -> tuple[PackFile, ...]:
    ...
    value = json.loads(raw, object_pairs_hook=_no_duplicates)
    if (
        value.get("schema_version") != "ac-alpha-staging-films.v1"
        or value.get("manifest_id") != "alpha-public-films-12s-v1"
        or value.get("fixture_registry_sha256") != REGISTRY_SHA256
        or value.get("production_enabled") is not False
        or value.get("course_content") is not False
...
def install_pack(release: Path, environment: str, source: Path) -> str:
    if environment != "staging":
        raise _fail("installation is staging-only")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(
            "FAIL  Staging public-film release policy or immutable package validation failed.",
            file=sys.stderr,
        )
        return 2
    return 0
