...
- Docker ingress rules are deterministically reinserted as the first two `DOCKER-USER` rules and fixture-tested against a terminal-`RETURN` bypass. Baseline promotion uses a rollback-safe managed hold transition and records the prior hold set beside each host backup.
