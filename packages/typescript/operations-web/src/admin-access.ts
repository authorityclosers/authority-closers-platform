...
function isServerOwnedAdminContext(
  ...
  return (
    ...
    (candidate.permissions.includes("admin_surface") ||
      (Array.isArray(candidate.studioCapabilities) &&
        candidate.studioCapabilities.length > 0 &&
        candidate.studioCapabilities.every(
          (capability) =>
            studioCapabilitySchema.safeParse(capability).success &&
            capability.tenant_id === candidate.tenantId,
        )))
  );
}
...
export function canAccessAdminPath(
  context: ServerOwnedAdminContext,
  pathname: string,
): boolean {
  if (!isServerOwnedAdminContext(context)) return false;
  if (context.permissions.includes("admin_surface")) return true;
  return (
    pathname === "/" ||
    pathname === "/catalog" ||
    ...
    /^\/v1\/admin\/studio(?:\/|$)/.test(pathname) ||
    /^\/v1\/admin\/program-versions\/[^/]+\/publish$/.test(pathname)
  );
}

export function renderPermissionDeniedDocument(studioOnly = false) {
  return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8">
  ...
  <body>
    ...
    <main id="admin-denied" tabindex="-1">
      ...
      <h1>${studioOnly ? "This page is outside your Studio access." : "Authenticated admin context required."}</h1>
      <p>${studioOnly ? "Your current assignment gives you access to Academy Studio. People and operations require separate admin permissions." : "This server has no verified, server-owned admin actor and tenant context. The admin surface is not published."}</p>
      ${studioOnly ? '<a class="sign-in" href="/studio/programs">Open Academy Studio</a>' : '<p>Client role assertions, request parameters, and headers cannot unlock this route.</p><a class="sign-in" href="/v1/auth/google/start?action=authenticate&amp;surface=admin&amp;return_path=%2F">Sign in with Google</a>'}
    </main>
  </body>
</html>`;
}
