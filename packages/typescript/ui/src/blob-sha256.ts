/** Bounded browser hashing shared by avatar and lecture uploads. No file bytes persist. */
const SHA256_K = [
...
export const BLOB_HASH_CHUNK_BYTES = 256 * 1024;
...
export async function hashBlobSha256(
  ...
  const chunkBytes = BLOB_HASH_CHUNK_BYTES;
  ...
  for (let offset = 0; offset < file.size; offset += chunkBytes) {
    ...
    if (bytes.byteLength !== length)
      throw new TypeError("The file could not be read completely.");
    hash.update(new Uint8Array(bytes));
    onProgress?.((offset + length) / file.size);
    // Yield between bounded pieces so a large lecture never monopolizes input/paint.
