...
import {
  requestReviewerServerIdentity,
  resolveReviewerServerContext,
} from "./reviewer-server-auth";
...
it("does not call the internal API with the normal Admin cookie", async () => {
  ...
  await expect(
    resolveReviewerServerContext({
      ...base,
      production: true,
      cookieHeader: "__Host-ac_session=" + "a".repeat(43),
      fetcher,
    }),
  ).resolves.toBeNull();
...
it("admits a verified dedicated reviewer cookie from the internal reviewer projection", async () => {
  ...
  await expect(
    resolveReviewerServerContext({
      ...base,
      production: true,
      cookieHeader: "__Host-ac_reviewer_session=" + "r".repeat(43),
      fetcher,
    }),
  ...
  expect(fetcher).toHaveBeenCalledWith(
    new URL("http://api.staging.ac.internal.invalid:8000/v1/reviewer/me"),
    expect.objectContaining({
      headers: expect.objectContaining({
        cookie: "__Host-ac_reviewer_session=" + "r".repeat(43),
...
it("preserves the canonical Admin Host on the native loopback transport", async () => {
  ...
  try {
    ...
    const response = await requestReviewerServerIdentity(
      new URL(`http://127.0.0.1:${address.port}/v1/reviewer/me`),
