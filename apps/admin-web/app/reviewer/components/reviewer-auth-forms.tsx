...
export function ReviewerLoginForm() {
  ...
  return (
    <AuthCard
      eyebrow="Dedicated access"
      title="Sign in to review"
      description="Use the email address that received reviewer access. Reviewer sign-in is separate from your Academy learner session."
    >
      ...
      <p className={styles.note}>
        Reviewer links are one-time and expire. Open the link in this same
        browser after checking your inbox.
      </p>
    </AuthCard>
  );
}
...
export function ReviewerVerification() {
  ...
  const requestRef = useRef<Promise<
    Awaited<ReturnType<typeof verifyReviewerSignIn>>
  > | null>(null);
  const [status, setStatus] = useState<
    "loading" | "missing" | "verifying" | "error"
  >("loading");
  const [message, setMessage] = useState("");
  useEffect(() => {
    let active = true;
    ...
    void requestRef.current.then(
      async (verified) => {
        ...
        if (!session) {
          setStatus("error");
          setMessage(
            "The reviewer session cookie was not established. Request a fresh sign-in link.",
          );
          return;
        }
        const destination = verified.assignment_id
          ? `/reviewer/review/${encodeURIComponent(verified.assignment_id)}`
          : "/reviewer";
  ...
  return (
    <AuthCard
      ...
      title="Verifying your reviewer link"
      description="Open this link in the same browser where you requested reviewer access."
    >
      ...
      {status === "missing" || status === "error" ? (
        <>
          ...
          <Link className={styles.secondaryButton} href="/reviewer/login">
...
export function ReviewerInvitationAcceptance() {
  ...
  const tokenRef = useRef<string | null>(null);
  const requestRef = useRef<Promise<
    Awaited<ReturnType<typeof acceptReviewerInvitation>>
  > | null>(null);
  const [email, setEmail] = useState("");
  const [authStatus, setAuthStatus] = useState<"idle" | "sending" | "sent">(
    "idle",
  );
  const [status, setStatus] = useState<
    "loading" | "email" | "accepting" | "accepted" | "error"
  >("loading");
  const [message, setMessage] = useState("");
  ...
  const requestAccess = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setAuthStatus("sent");
      setMessage(
        "If the invitation matches that address, check your inbox for a reviewer sign-in link.",
      );
    } catch (error) {
      setAuthStatus("idle");
      setMessage(reviewerAuthErrorMessage(error));
  ...
  return (
    <AuthCard
      ...
      title="Claim your assigned review"
      description="Verify the email address that received this invitation. A new reviewer account is admitted to this workspace without changing Academy learner access."
    >
      {displayStatus === "loading" || displayStatus === "accepting" ? (
        <p className={styles.status} role="status">
          {displayStatus === "loading"
            ? "Checking your reviewer session…"
            : "Opening the assigned review…"}
        </p>
      ) : null}
      {displayStatus === "email" ? (
        <form className={styles.form} onSubmit={requestAccess}>
          <label htmlFor="invited-reviewer-email">Invitation email</label>
          <input
            id="invited-reviewer-email"
            type="email"
            autoComplete="email"
            value={email}
            onChange={(event) => setEmail(event.target.value)}
          ...
          >
            {authStatus === "sending" ? "Sending link…" : "Continue with email"}
          </button>
          {message ? (
      ...
      {displayStatus === "accepted" ? (
        <p className={styles.success} role="status">
          Invitation accepted. Opening the review…
        </p>
      ) : null}
      <p className={styles.note}>
        Open the sign-in link in this same browser after checking your inbox.
      </p>
    </AuthCard>
  );
}
