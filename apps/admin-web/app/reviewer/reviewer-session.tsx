...
const ReviewerSessionContext =
  createContext<ReviewerSessionContextValue | null>(null);
...
export function ReviewerSessionProvider({
  ...
  const [state, setState] = useState<ReviewerSessionState>({
    status: "loading",
    session: null,
  });
      ...
      setState({
        ...
        message:
          error instanceof Error
            ? error.message
            : "Reviewer session could not be checked.",
      });
      return null;
    }
  }, []);
    ...
    try {
      ...
      setState((previous) => ({
        ...
        message:
          error instanceof Error
            ? error.message
            : "Reviewer sign-out could not be confirmed.",
      }));
      return false;
    }
  }, []);
  ...
  return (
    <ReviewerSessionContext.Provider value={value}>
      {children}
    </ReviewerSessionContext.Provider>
  );
}
...
export function useReviewerSession() {
  ...
  if (!value)
    throw new Error(
      "useReviewerSession must be used below ReviewerSessionProvider.",
    );
  return value;
}
...
export function ReviewerShell({ children }: { children: React.ReactNode }) {
  ...
  const assignmentRoute =
    pathname === "/reviewer/review" || pathname.startsWith("/reviewer/review/");
    ...
    <div className={styles.shell}>
      <header className={styles.header}>
        <Link
          ...
          href="/reviewer"
          aria-label="Reviewer workspace home"
        >
          <span className={styles.brandMark} aria-hidden="true">
            AC
          </span>
          <span>
            <strong>Authority Closers</strong>
      ...
      {assignmentBlocked ? (
        <main id="admin-content" className={styles.boundary} tabIndex={-1}>
          ...
          >
            ...
            <p>
              {status === "loading"
                ? "Your assigned source will open after access is verified."
                : status === "error"
                  ? (sessionError ??
                    "The dedicated reviewer session could not be checked.")
                  : "This assigned review is private to the signed-in reviewer."}
            </p>
            {status === "error" ? (
              <button
            ...
            {status === "signed_out" || status === "error" ? (
              <Link className={styles.boundaryButton} href="/reviewer/login">
