...
const assignment = z
  .object({
    ...
    state: z.enum(["assigned", "in_progress", "submitted", "revoked", "expired"]),
  ...
  .superRefine((value, context) => {
    if (new Set(value.allowed_lenses).size !== value.allowed_lenses.length)
      context.addIssue({ code: "custom", path: ["allowed_lenses"], message: "Assignment lenses must be unique." });
    if (value.expires_at_epoch <= value.created_at_epoch)
      context.addIssue({ code: "custom", path: ["expires_at_epoch"], message: "Assignment expiry is invalid." });
    if (
      ...
      context.addIssue({ code: "custom", path: ["checkpoint"], message: "Assignment source binding is invalid." });
    }
  });
...
async function requestJson<T>(
  ...
  if (!path.startsWith("/v1/reviewer/") || path.startsWith("//"))
    throw new TypeError("Reviewer requests must use the dedicated same-origin API.");
...
export function reviewerErrorMessage(error: unknown): string {
  if (error instanceof ReviewerApiProblem) {
    if (error.status === 401) return "Your reviewer session expired. Request a fresh sign-in link.";
    if (error.status === 403) return "This reviewer access is no longer active.";
    if (error.status === 404) return "This review is unavailable for the signed-in reviewer.";
    if (error.status === 409) return "The assigned call revision changed. Your draft is preserved.";
  }
...
function assertBinding(value: ReviewAssignmentBinding, id: string) {
  if (
    ...
    value.checkpoint.source_sha256 !== value.source.source_sha256 ||
    value.checkpoint.source_revision !== value.source.source_revision
  ) throw new Error("The reviewer service returned a different assignment binding.");
  return value;
}
...
function toFormAssignment(
  ...
  const citations = (items: z.infer<typeof citation>[]) => items.map((item) => `${item.doc}: ${item.sections.join(", ")}`);
  return {
    ...
    reviewer: { person_id: bound.reviewer_person_id, display_name: "Assigned reviewer" },
    report: {
      ...
      groups: [
        ...groups.map(([title, findings]) => ({
          ...
          findings: findings.map((item) => ({
            ...
            evidence: item.evidence.map((ref) => `${ref.segment_id} · ${ref.start_ms / 1000}–${ref.end_ms / 1000}s: ${ref.quote}`),
          })),
        })),
        {
...
function parseSaved(value: unknown, loaded: LoadedReviewAssignment): SavedReview {
  ...
  if (
    ...
    item.evidence_refs.some((ref) => ref.checkpoint_id !== bound.checkpoint.id || !loaded.assignment.clips.some((clip) => clip.segment_id === ref.span_id)) ||
    new Set(item.evidence_refs.map((ref) => ref.span_id)).size !== item.evidence_refs.length ||
    (item.proposed_correction && !(CORRECTION_TARGETS[item.lens] as readonly string[]).includes(item.proposed_correction.target_layer))
  ) throw new Error("The saved reviewer feedback differs from this assignment.");
  return item;
}

export function createReviewerAssignmentApi(fetcher: Fetcher = fetch) {
  return {
    async queue(signal?: AbortSignal): Promise<{ items: ReviewerAssignmentSummary[]; truncated: boolean }> {
      return requestJson("/v1/reviewer/review-assignments?limit=50", { method: "GET", signal }, z.object({ items: z.array(assignment), truncated: z.boolean() }), fetcher);
    },
    async get(assignmentId: string, signal?: AbortSignal): Promise<LoadedReviewAssignment> {
      const id = uuid.parse(assignmentId);
      const payload = loadedResponse.parse(await requestJson(`${endpoint(id)}`, { method: "GET", signal }, loadedResponse, fetcher));
      const bound = assertBinding(payload.assignment, id);
      if (payload.audio_source_url !== `${endpoint(id)}/source` || payload.report.source_sha256 !== bound.source.source_sha256 || payload.transcript.source_sha256 !== bound.source.source_sha256 || payload.evidence_spans.some((span) => span.checkpoint_id !== bound.checkpoint.id)) {
        throw new Error("The report, transcript, or audio source is not bound to this assignment.");
      }
      return { binding: bound, transcript: payload.transcript, assignment: toFormAssignment(bound, payload.report, payload.evidence_spans, payload.audio_source_url) };
    },
    async history(loaded: LoadedReviewAssignment, signal?: AbortSignal): Promise<SavedReview[]> {
      const payload = await requestJson(`${endpoint(loaded.binding.id)}/submissions`, { method: "GET", signal }, z.object({ items: z.array(z.unknown()).max(100) }), fetcher);
      const items = payload.items.map((item) => parseSaved(item, loaded));
      if (new Set(items.map((item) => item.id)).size !== items.length) throw new Error("Saved reviewer feedback contains duplicate submissions.");
      return items;
    },
    async submit(loaded: LoadedReviewAssignment, draft: ReviewProposalDraft, idempotencyKey: string): Promise<SavedReview> {
      const bound = loaded.binding;
      if (!draft.clip || draft.assignment_id !== bound.id || draft.reviewer_id !== bound.reviewer_person_id || draft.clip.checkpoint_id !== bound.checkpoint.id || !loaded.assignment.clips.some((clip) => clip.segment_id === draft.clip?.segment_id)) throw new Error("Choose one server-provided evidence span before saving.");
      const body = {
        schema: "ac.sales-xray.review-feedback/1" as const,
        idempotency_key: boundedId.parse(idempotencyKey),
        lens: lens.parse(draft.lens),
        evidence_refs: [{ checkpoint_id: draft.clip.checkpoint_id, span_id: draft.clip.segment_id }],
        confidence: confidence.parse(draft.confidence),
        feedback: z.string().trim().min(1).max(4000).parse(draft.feedback),
        ...(draft.proposed_correction ? { proposed_correction: correction.parse(draft.proposed_correction) } : {}),
      };
      const saved = parseSaved(await requestJson(`${endpoint(bound.id)}/submissions`, { method: "POST", headers: { "content-type": "application/json", accept: "application/json", "idempotency-key": idempotencyKey }, body: JSON.stringify(body) }, submission, fetcher), loaded);
      if (saved.idempotency_key !== idempotencyKey || saved.lens !== body.lens || saved.confidence !== body.confidence || saved.feedback !== body.feedback || JSON.stringify(saved.evidence_refs) !== JSON.stringify(body.evidence_refs) || JSON.stringify(saved.proposed_correction ?? null) !== JSON.stringify(body.proposed_correction ?? null)) throw new Error("The server receipt differs from your feedback. Retry unchanged feedback to verify it.");
      return saved;
    },
  };
}
...
export async function acceptReviewerInvitation(token: string, fetcher: Fetcher = fetch): Promise<ReviewAssignmentBinding> {
  return requestJson("/v1/reviewer/review-invitations/accept", { method: "POST", headers: { "content-type": "application/json", accept: "application/json" }, body: JSON.stringify({ schema: "ac.sales-xray.review-invitation-accept/1", token }) }, assignment, fetcher);
}
