...
const opaqueToken = z
  ...
  .min(40)
  .max(512)
  .refine((value) => !/\s/u.test(value), "Token is malformed.");
...
async function request<T>(
  ...
  if (!path.startsWith("/v1/reviewer/") || path.startsWith("//"))
    throw new TypeError("Reviewer auth must use a dedicated same-origin path.");
  ...
  if (!response.ok) {
    ...
    throw new ReviewerApiProblem(
      ...
      typeof candidate.detail === "string" && candidate.detail.trim()
        ? candidate.detail
        : "The reviewer sign-in request was rejected.",
  ...
  try {
    ...
    throw new Error("The reviewer service returned an empty response.");
  }
  return schema.parse(body);
}

export function loadReviewerSession(
  signal?: AbortSignal,
): Promise<ReviewerIdentity | undefined> {
  return request(
    "/v1/reviewer/me",
    { method: "GET", signal },
...
export function requestReviewerSignIn(
  emailAddress: string,
  ...
  const body = {
    ...
    ...(invitationToken
      ? { invitation_token: opaqueToken.parse(invitationToken) }
      : {}),
  };
  return request("/v1/reviewer/auth/request", {
    method: "POST",
...
export function verifyReviewerSignIn(
  token: string,
): Promise<ReviewerVerification> {
  return request(
    "/v1/reviewer/auth/verify",
    {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ token: opaqueToken.parse(token) }),
    },
    reviewerVerificationSchema,
  ) as Promise<ReviewerVerification>;
}

export function logoutReviewer(): Promise<void> {
  return request("/v1/reviewer/auth/logout", { method: "POST" }).then(
    () => undefined,
  );
}
...
export function reviewerAuthErrorMessage(error: unknown): string {
  if (error instanceof ReviewerApiProblem) {
    if (error.status === 401)
      return "That sign-in link is invalid or expired. Request a fresh link.";
    if (error.status === 403)
      return "Reviewer access is not active for this address.";
  ...
  return error instanceof Error && !(error instanceof z.ZodError)
    ? error.message
    : "Reviewer sign-in could not be completed.";
}
