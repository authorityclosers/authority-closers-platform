...
function TranscriptPanel({ value }: { value: Transcript }) {
  ...
  const rows = value.segments.filter((segment) =>
    `${segment.speaker_id} ${segment.text} ${formatTime(segment.start_ms)}`
      .toLowerCase()
  ...
  return (
    <section
      ...
      aria-labelledby="full-transcript-title"
    >
      ...
      >
        ...
        {rows.length === 0 ? (
          <p className={styles.empty}>No transcript lines match this search.</p>
        ) : null}
      </div>
      <p className={styles.integrityNote}>
        Transcript revision {value.revision}. Source text is bound to this
        assignment and remains read-only.
      </p>
    </section>
  );
}

export function ReviewerAssignmentAdapter({
  assignmentId,
}: {
  assignmentId: string;
}) {
  return (
    <ReviewerAssignmentSession key={assignmentId} assignmentId={assignmentId} />
  );
}
...
function ReviewerAssignmentSession({ assignmentId }: { assignmentId: string }) {
  ...
  useEffect(() => {
    ...
    void api
      ...
      .then((value) => {
        if (controller.signal.aborted || current.current !== assignmentId)
          return;
        setLoaded(value);
    ...
    <main id="admin-content" className={styles.main} tabIndex={-1}>
      <div className={styles.container}>
        <div className={styles.breadcrumb}>
          Reviewer workspace / Assigned review
        </div>
        {loaded?.binding.id === assignmentId ? (
          <>
            <div className={styles.assignmentHeader}>
              <div>
                ...
                <p>
                  Listen to the exact call, inspect the report and transcript,
                  then leave feedback through the lens you were assigned.
                </p>
              </div>
              <dl className={styles.assignmentFacts}>
                <div>
                  <dt>Assignment</dt>
                  <dd>{loaded.binding.id.slice(0, 8)}…</dd>
                </div>
                <div>
                  <dt>Expires</dt>
                  <dd>
                    {new Date(
                      loaded.binding.expires_at_epoch * 1000,
                    ).toLocaleString()}
                  </dd>
                </div>
                <div>
                  <dt>Lenses</dt>
                  <dd>{loaded.binding.allowed_lenses.join(" · ")}</dd>
                </div>
              </dl>
            </div>
            <AssignedReviewForm
              ...
              verifyAudioAccess={async () => {
                try {
                  ...
                  throw new Error(reviewerErrorMessage(reason));
                }
              }}
            ...
            >
                ...
                <article key={item.id} className={styles.historyItem}>
                  <div className={styles.historyItemHeading}>
                    ...
                    <span>
                      ...
                      {new Date(item.created_at_epoch * 1000).toLocaleString()}
                    </span>
                  </div>
                  <p>{item.feedback}</p>
                  <small>
                  ...
                  {item.proposed_correction ? (
                    <dl className={styles.correction}>
                      <div>
              ...
              {history.length >= 100 ? (
                <p className={styles.integrityNote}>
                  The service returns at most 100 saved reviews. Newly confirmed
                  feedback remains visible during this visit.
                </p>
              ) : null}
            </section>
          </>
          ...
          <section className={styles.stateCard} aria-label="Assignment status">
            {error ? (
              ...
              <p role="status">
                Loading your assigned report, transcript and evidence…
              </p>
            )}
          </section>
        )}
