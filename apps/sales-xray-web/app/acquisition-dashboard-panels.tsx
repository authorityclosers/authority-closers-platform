    ...
    <aside className={styles.rail} aria-label="Getting started and help" data-static-preview={staticPreview}>
        ...
        <ol className={styles.stepList}>
          {steps.map((step, index) => (
            <li className={styles.step} key={step.title}>
              <span className={`${styles.stepMark} ${completed.has(index) ? styles.stepDone : ""}`} aria-hidden="true">
        ...
        {!staticPreview && <div className={styles.helpCard}>
          ...
          <details>
            ...
            <ul className={styles.helpTips}>
              ...
              <li>Check the language and privacy details before you continue.</li>
      ...
      <section className={`${styles.card} ${styles.sampleCard}`} aria-labelledby="sample-insight-title">
        ...
        <div className={styles.sampleQuote}>
          ...
          <p>A prospect raises a pricing concern. The analysis points to that moment so you can review the response and plan a follow-up.</p>
        </div>
        <div className={styles.sampleMoments}>
          <strong>What a report can highlight</strong>
          <span><i className={styles.mintDot} />Buying signals</span>
          <span><i className={styles.orangeDot} />Objections</span>
    ...
    <div className={styles.lowerPanels} data-compact={compact}>
      <section className={styles.card} aria-labelledby="saved-calls-title">
        <div className={styles.lowerHeading}>
          ...
          <Link href="/calls">View all <ArrowRight size={16} aria-hidden="true" /></Link>
        </div>
        <div className={styles.callColumns} aria-hidden="true">
          <span>Call name</span><span>Date</span><span>Duration</span><span>Status</span>
        </div>
        {calls.length ? (
          <ul className={styles.callList}>
            {calls.map((call) => (
              <li className={styles.callRow} key={call.id}>
                <span className={styles.rowIcon}><AudioLines size={18} aria-hidden="true" /></span>
                <span className={styles.callName}>
                  {call.href ? <Link href={call.href}>{call.title}</Link> : <strong>{call.title}</strong>}
          ...
          <div className={styles.emptyState}>
            <span className={styles.emptyIcon}><FolderOpen size={24} strokeWidth={1.7} aria-hidden="true" /></span>
      ...
      <section className={styles.card} aria-labelledby="recent-activity-title">
        ...
        {activity.length ? (
          <ul className={styles.activityList}>
            {activity.map((item) => (
              <li className={styles.activityRow} key={item.id}>
                <span className={styles.rowIcon}>{item.kind === "saved" ? <FileText size={18} aria-hidden="true" /> : <AudioLines size={18} aria-hidden="true" />}</span>
          ...
          <div className={styles.emptyState}>
            <span className={styles.emptyIcon}><Clock3 size={24} strokeWidth={1.7} aria-hidden="true" /></span>
