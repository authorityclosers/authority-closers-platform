...
it("mounts the real static processing panel after local health with no operational request", async () => {
  ...
  expect(
    container.querySelector('[aria-label="Example analysis progress"]'),
  ).not.toBeNull();
  expect(
...
it("renders one or three local synthetic Files in the production stage without operational requests", async () => {
  ...
  expect(
    container.querySelectorAll('[aria-label="Added audio files"] li'),
  ).toHaveLength(1);
  ...
  expect(
    container.querySelector('[data-preview-mode="multiple"]'),
  ).not.toBeNull();
  expect(
    container.querySelectorAll('[aria-label="Added audio files"] li'),
  ).toHaveLength(2);
