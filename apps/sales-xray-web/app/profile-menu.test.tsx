...
it("shows the canonical profile name in the signed-in menu", async () => {
  ...
  expect(host.querySelector('[role="region"]')?.textContent).toContain("Morgan Lee");
...
it("keeps a neutral label when the canonical profile has no name", async () => {
  ...
  expect(host.querySelector<HTMLButtonElement>("button[aria-expanded]")?.textContent).toContain("AC account");
});
it("refreshes the name after a confirmed profile update", async () => {
  ...
  expect(host.querySelector<HTMLButtonElement>("button[aria-expanded]")?.textContent).toContain("Morgan Lee");
  await act(async () =>
    window.dispatchEvent(new Event(PROFILE_UPDATED_EVENT)),
  );
  expect(host.querySelector<HTMLButtonElement>("button[aria-expanded]")?.textContent).toContain("Alex Rivera");
});
