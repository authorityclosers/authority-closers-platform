...
import { AcquisitionFileStage, type AcquisitionFileStageProps } from "./acquisition-file-stage";

(globalThis as typeof globalThis & { IS_REACT_ACT_ENVIRONMENT: boolean }).IS_REACT_ACT_ENVIRONMENT = true;

const first = new File([new Uint8Array(1024)], "Discovery call.m4a", { type: "audio/mp4", lastModified: 1 });
const second = new File([new Uint8Array(512)], "Follow-up call.wav", { type: "audio/wav", lastModified: 2 });
const third = new File([new Uint8Array(256)], "Closing call.mp3", { type: "audio/mpeg", lastModified: 3 });

let host: HTMLDivElement;
let root: Root;
let callbacks: Pick<AcquisitionFileStageProps, "onSelect" | "onRemove" | "onClear" | "onAddFiles">;
...
it("shows local file facts and selects or removes one file without claiming a duration", async () => {
  ...
  expect(host.textContent).toContain("Discovery call.m4a selected for analysis");
  ...
  await act(async () => host.querySelector<HTMLButtonElement>('button[aria-label="Remove Discovery call.m4a"]')!.click());
  expect(callbacks.onRemove).toHaveBeenCalledWith(first);
  await act(async () => [...host.querySelectorAll<HTMLButtonElement>("button")].find((button) => button.textContent === "Clear all")!.click());
...
it("forwards dropped files locally without an upload or analysis action", async () => {
  ...
  await act(async () => host.querySelector('[role="group"]')!.dispatchEvent(drop));
...
it("disables browse, selection, removal and drop during an active action", async () => {
  await renderStage({ files: [first, second], selectedFile: first, disabled: true });
  expect([...host.querySelectorAll<HTMLButtonElement>("button")].filter((button) => !button.disabled)).toHaveLength(0);
  const drop = new Event("drop", { bubbles: true, cancelable: true });
