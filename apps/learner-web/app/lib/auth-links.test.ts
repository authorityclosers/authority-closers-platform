...
describe("Google authorization links", () => {
  ...
  it("uses a distinct server-owned registration intent", () => {
    ...
    expect(url.searchParams.get("action")).toBe("register");
    expect(url.searchParams.get("return_path")).toBe(
      GOOGLE_REGISTRATION_RETURN_PATH,
    );
  });

  it("keeps one authoritative Google registration return path", () => {
    expect(GOOGLE_REGISTRATION_RETURN_PATH).toBe("/onboarding");
    expect(
      new URL(
        googleAuthStartUrl("register"),
        "https://app.authorityclosers.com",
      ).searchParams.getAll("return_path"),
    ).toEqual([GOOGLE_REGISTRATION_RETURN_PATH]);
  });
