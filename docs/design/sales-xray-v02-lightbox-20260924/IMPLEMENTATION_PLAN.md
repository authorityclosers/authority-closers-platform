...
| P1    | Accepted as foundations only | Codex validation and independent source review passed; see §18. This intermediate shell is not a release candidate. |
| P2–P6 | Not started                  |                                                                                                                     |
...
## 16. P1 implementation notes (Claude, 25 September 2026)

- **Tokens:** `app/lightbox/tokens.css` is the corrected derivative; the package file is unchanged. It applies the §15.1 corrections plus the exceptions listed in its header, all needed so that every asserted small-text pair reaches 4.5:1 in light and dark, including alpha fills:
  - `--lx-objection-ink` (dark #FF8CAE; the source dark pair measures 4.42:1),
  - `--lx-on-teal`,
  - dark info/warning/hypothesis values,
  - `--lx-glow-ink` (referenced by the Lens assets),
  - film Lens colours,
  - a `[data-script="deva"]` rule.

  `tokens.test.ts` asserts the documented differences, the source failures they fix, the contrast pairs, embed parity and no raw hex in new modules.

- **Embed isolation:** light tokens are mirrored into `styles.css` under `:where(html:not([data-lx-root])) .xray-app`. The standalone `<html data-lx-root>` skips that block, so its theme comes from `tokens.css` alone and learner-web always gets light tokens. The `@scope (.xray-app)` boundary is unchanged.
- **Theme:** System/Light/Dark infrastructure ships now:
  - a pre-paint script at the top of `<body>`,
  - a local preference, tolerant of blocked storage,
  - live System following,
  - cross-tab sync.

  The control lives in the account menu. It is released only in non-production builds or with `AC_SALES_XRAY_THEME_PREVIEW=1`. Production stays light, with no script, until the P6 matrix passes (decision 12). Legacy surfaces not yet on tokens stay light inside a dark shell, so dark mode is not validated beyond the P1 primitives and shell.

- **Fonts:** the five `wght.css` entries are imported in `layout.tsx`. `--font-sans`, `--font-plus-jakarta` and `--report-font` now resolve to `--lx-font-ui` in the standalone document. Georgia and Cascadia remain in legacy modules until their surfaces migrate. Quotes use upright Newsreader, because italics are not loaded.
- **Shell:** `AcquisitionShell` keeps its export and props and renders `shell/lightbox-shell.tsx`, with one optional addition, a verified `allowance` for the minutes meter. Changes:
  - Removed: the greeting banner, slogan, "Need help?" card and footer CTA.
  - Help and legal links move to the `?` menu.
  - Page headings are compact, with no ETA copy.
  - The account row reuses `ProfileMenu`.
  - The bottom navigation shows below 900 px (the spec said 620 px), because there is no rail between 620 and 899 px.
  - Chrome joins the document flow at heights of 560 px or less.
  - The "Account" link keeps its current destination and label until P5 (Q4).
- **Assets:** verbatim copies are in `public/lightbox/`. Glyphs and the Lens are inline React SVG with per-instance IDs. The old `acquisition-shell.module.css` is now unimported; it is left for P6 cleanup.

## 17. Next-phase decisions and owner feedback — 25 September 2026

The current staged reading report is the visual baseline the owner wants to keep improving. Preserve its full readable content, open section layout, semantic icons, reading flow and persistent player. Bring in the strongest Lightbox features without regressing that clarity. A new shell around the old intake is an intermediate implementation, not an accepted final screen or a release candidate.

P2 must replace the complete intake composition. One main heading and one primary upload action are sufficient. Remove the first-use empty Calls and Activity tables, Get started checklist, Sample insight sidebar and duplicate page/body headings. Use the supplied Home, Confirm and Mobile-Upload boards for hierarchy and spacing; apply the current product decisions and actual contracts. Returning users may see their real recent calls. Keep selected media prominent, retain account/profile/consent gates and keep New analysis reachable.

For P2, retain the existing fetch transport and its enforced `redirect: "error"` behavior. Do not introduce XHR until an equivalent enforced destination boundary has been demonstrated. Upload status must be honestly indeterminate when acknowledged byte progress is unavailable; show a measured percentage only when the actual transport supplies it. This is a scoped transport decision, not a reason to postpone the intake redesign. Keep recovery-read-before-replay and exact request headers. Test pending-versus-promoted submission IDs, authentication recovery, consent continuity and no accidental replay.

Background upload continuity may be claimed only after a root-owned provider preserves the live transport across client navigation. Do not replace that with full reload links, localStorage promises or an animated indicator disconnected from the upload. If root layout integration needs a small source edit, request it from Codex. Server processing is independently tracked from the browser upload. Do not imply that closing a tab is safe before durable upload and dispatch confirmation.

The owner also prioritizes an admin user profile with audited grants of account minutes and a user request-for-more-access workflow. Those are separate backend/admin work; they must not be simulated in P2, and account-minute grants must not silently enlarge provider spending limits. The real provider comparison and verified production report remain release priorities alongside this UI work.

## 18. P1 acceptance and independent design review — 25 September 2026

Codex accepted P1 as a foundations checkpoint after typecheck, lint, formatter, build and the bounded complete test suites passed: 500 Sales Xray tests and 1,803 learner-web tests. The first unbounded test/build attempts exhausted this Windows machine's resources; their failure evidence is retained privately. The completed build used `RAYON_NUM_THREADS=2`, and the test reruns used at most two workers. No passing production or new-report-quality claim follows from these local checks.

The independent source review found no blocking P1 defect and confirmed shell API compatibility, real-allowance-only display, production theme gating and embed isolation. The existing full-document New analysis navigation remains a P2 prerequisite, as described above. Browser checks covered the light/dark desktop shell, expanded/collapsed rail, and profile menus at 390×844 and 320×568. A real clipping defect in the collapsed account popover was corrected by removing the rail's internal clipping; short viewports retain document flow. Root also corrected the blocked-storage tests, updated navigation assertions, preserved all six font license files and removed server-oriented processing copy. The legacy intake body is deliberately not accepted by this checkpoint.

An independent AGY Gemini review supports a single focused intake column, a prominent selected-file confirmation, clear upload/processing distinction, persistent audio and preserved reading/tab state. Use these as design guidance with the supplied boards and current tokens. Several suggestions were rejected: an invented 250 MB limit, changed consent/legal wording, unverified voice-signature attribution, generic Retry upload after an unknown outcome, arbitrary qualitative ratings and hiding overflow to conceal layout defects. Existing product contracts and §§15–17 govern all of these. The review is advice, not implementation or browser evidence.
