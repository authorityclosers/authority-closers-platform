...
Original path: `docs/evidence/20260910_ACADEMY_COMMUNITY_IDENTITY_CANDIDATE.md`. This separately preserved candidate record does not replace the existing main record or establish acceptance of the reconciled release. The original candidate content follows unchanged.
...
## Candidate recovery-policy correction

Broader release verification found that migration0024's index-drop downgrade violated the repository-wide forward-only recovery contract. Its downgrade now deliberately raises `RuntimeError`; its revision, predecessor and upgrade are unchanged. Migration0027 already refused downgrades, but its error message now explicitly says `forward-only` to match that same policy check. No applied upgrade was rewritten or renumbered, no database downgrade was run, and the policy test was not weakened. The focused policy check passed; the subsequent combined Coach/upload/restore regression passed174 with two explicitly unavailable restore integrations. Sol was instructed to include only these exact corrections in the candidate.

## Corrected immutable candidate

The initial `99fdc62bf43ce1206871502cb58176cf1eb2214e` candidate was not accepted or deployed. Independent review required two corrections: a leaderboard retry must not abort an in-flight username/participation mutation, and domain mutations must require an explicit caller-owned database transaction. Both now have focused regressions. The replacement also validates cursor Base64 input, labels the current learner for screen readers, and bounds raw username input to128 characters while retaining normalized3–30 domain rules.

The replacement candidate is `d1c9ba754f99a110ba92cce98a0af6858df49f40`, parent `783da9838179ee32f0481d9bae9a8b1b30a95ec4`. Root inspected its corrective diff and applied only the seven changed source/test files to the authoritative working tree. Their Git blob identities exactly match the replacement; local focused Python community/HTTP verification passed50. This does not imply the entire dirty working tree equals the candidate. Separate local browser acceptance and immutable packaging/deployment evidence must be recorded when complete.

The first dispatched run, `34414281688`, failed the repository-wide Ruff format check on three allowlisted Python files; earlier migration, database privilege, backup-role and PostgreSQL checks passed. The normal follow-up commit `74631e0dd94a1d54a2e2b88bc925e47e0e0c7029` contains mechanical formatting only, with no foundation delta. Root applied the same formatter and confirmed exact Git blob equality for all three files. Replacement workflow `34414740852` is the release run to verify; no deployment is established merely by its dispatch.

Corrected-source normal-browser run `.tmp/proof/community-identity-qa-d1c9ba/run-20260909T224917Z/proof.json` passed with fourteen screenshots, 320/390/768/1440 light/dark, saved identity and participation, normal login, reload, GET-only failure/retry/slow/offline fixtures, and final synthetic-learner opt-out. Its current-learner row includes the accessible `(you)` label. This is source-integration evidence, not whole-worktree/candidate equivalence or production acceptance. Fixed-navigation placement in a full-page screenshot requires a separate viewport interaction check before being classified as an inaccessible-control defect.

Read-only VPS inventory before promotion confirmed active foundation `foundation-7bdd069d4bf00561edcab90b1679666205708185`, current staging `783da9838179ee32f0481d9bae9a8b1b30a95ec4`, active logical-backup timer, healthy application/foundation containers, and159GiB free disk. The foundation delta to the replacement consists only of the two reviewed versioned-parity helpers; no Docker configuration, installer, firewall or route-source change is included. A canonical verified foundation release is still required before applying migrations0026/27. No remote change is claimed by this inventory.

## Immutable package and foundation acceptance

Workflow `34414740852` succeeded for exact candidate `74631e0dd94a1d54a2e2b88bc925e47e0e0c7029`, including validation, academy-capacity simulation and image packaging. Artifact `10128976187`, named `ac-application-74631e0dd94a1d54a2e2b88bc925e47e0e0c7029`, is unexpired and has digest `sha256:4563a4818d0890989b36d50ef0046b545bdb07b5b197cac8ef78472620f527ee`. Root's post-format community unit/HTTP plus real PostgreSQL run passed57; Ruff and format checks passed. Packaging is not production acceptance.

The viewport-only recheck `.tmp/proof/community-identity-viewport-qa-20260910/run-20260909T230823Z/proof.json` retracts the preliminary full-page screenshot obstruction finding: it was a stitching artifact. At320x844, Save spans y652.89–696.89 and helper text y708.89–771.89, above navigation starting y784; at390x844 the corresponding bounds are653.19–697.19 and709.19–772.19. Normal scrolling, edge/center hit tests, keyboard focus, join and withdrawal passed. The synthetic learner ended opted out.

Canonical foundation installation completed with exit0 and `STATIC_ACCEPTANCE_COMPLETE` at2026-09-09T23:15:05Z. The exact source archive SHA256 is `450fc24c436164eb552acdcf112e7c54aa84e982999e14623c161c33e70a8952`. The active foundation is now `foundation-74631e0dd94a1d54a2e2b88bc925e47e0e0c7029`. Its managed-file checks, private lock and timer-state restoration passed. Application containers were not restarted; edge/telemetry were recreated by the canonical installer. Pure read-only local/public health probes returned200 afterward. Staging remained on783 at this foundation-only acceptance point.

The retained pre-install host-file recovery archive is `/root/ac-bootstrap-backups/pre-community-foundation-20260909T231427Z.baaeid.tar.gz`, root-owned mode0600, 85,036,749bytes, SHA256 `d49924a0b110aef78245ceb6940a6b7746208d61de667914d06f5b5defeb7d21`. It preserves the55 managed files, two pinned binaries, toolchain marker and previous link; it is not a database or off-site backup.

Read-only journal inspection also found a preexisting logical-backup failure at `off_host_upload` with `diagnostics_unavailable`; the local dump was retained. Off-site recovery is therefore not accepted. The application installer still enforces its canonical pre-migration local dump/archive-list gate; no backup check was removed or weakened. Production remains blocked separately. No Cloudflare/DNS or WordPress changes were made in this foundation step. Application staging deployment has been started through `Deploy-Staging.ps1` for exact746; its completion must be recorded separately.

## Staging deployment completed

The canonical `Deploy-Staging.ps1 -ReleaseSha 74631e0dd94a1d54a2e2b88bc925e47e0e0c7029 -TargetEnvironment staging` completed with exit0. Public Learner/PWA assets, API liveness/readiness/programs, disabled API documentation, protected Admin routing, Coach normal-login routing, legacy-session recovery, exact Google OAuth-start callback/cookie constraints, and WordPress-preservation probes passed. OAuth-start acceptance is not an authenticated browser sign-in proof.

An independent SSH read confirmed current-staging points to exact746, all three web containers/API/PostgreSQL are healthy and the worker is running. Canonical receipt `/srv/authority-closers/application/deployments/staging/20260909T232238Z-74631e0dd94a1d54a2e2b88bc925e47e0e0c7029-yoRdcC.env` reports `AC_STATUS=COMMITTED` and migration head `20260910_0027`. The retained pre-migration database dump is `/srv/authority-closers/backups/application/staging/20260909T232143Z-pre-74631e0dd94a1d54a2e2b88bc925e47e0e0c7029.kpnd7t.dump`; the canonical installer verified its archive list before migration. This does not establish an off-site restore drill.

Independent unauthenticated public GET probes for `/v1/community/profile` and `/v1/community/leaderboard` returned401, not public learner data. All three staging app entry points were opened in controlled Chrome; Learner's existing browser session requires reauthentication, Coach presents normal sign-in, and Admin presents its Cloudflare Access gate. No real learner's username or participation was mutated for release proof. Authenticated remote community flow remains distinct from the successful synthetic localhost browser acceptance above.

Fresh production Learner/Coach probes still return503 release hold. WordPress apex/www both return200. No production application promotion was attempted, and the missing production owner configuration plus failed off-site backup remain unresolved. This release includes community identity/leaderboard and the allowlisted Arcade changes, not the unactivated Studio upload-to-processing pipeline or a completed4K course-delivery claim.

A bounded independent backup diagnostic found104 sanitized historical logical-upload failures since September7 (103 `diagnostics_unavailable`, one timeout), first at2026-09-08T05:59:44Z and latest at2026-09-09T23:14:26Z, definitively before the23:15:05 foundation acceptance. A new canonical timer run started23:20:07 and was still active at inspection; no success or failure is inferred for that run. The helper's `diagnostics_unavailable` classification means its bounded child-stderr drain did not complete, not proof of bad R2 credentials, DNS or provider authorization. No credential rotation or infrastructure alteration is justified by that label. A fresh successful canonical snapshot and restore proof remain outstanding; no jobs were launched by the diagnostic.

Follow-up of that exact invocation: `ac-postgres-backup.service` started2026-09-09T23:20:07.915787Z and completed with the sanitized systemd success marker at23:24:01.228226Z. Its window contained zero off-host-upload, diagnostics-unavailable, timeout, termination or command-failure markers. This supersedes the pending status for that invocation, not the historical failures or the still-unverified off-site restore drill. A later timer invocation starting23:50:15.917424Z was still active at inspection; no result is inferred for that later run. The follow-up was read-only and launched no backup job.
