...
The implementation refines the Direction A command-center architecture, synchronizes token hierarchies across light and dark modes, establishes a 280 px expanded / 76 px collapsed rail with smooth 180 ms transitions, introduces an overlay collapse toggle, renders body-portal floating tooltips in the collapsed state, styles the desktop utility header as a transparent, borderless 76 px surface, and updates the mobile brand header hierarchy.
...
- Displayed for collapsed navigation items and the collapsed Profile account item on hover or focus. Search, Settings, and Help retain their native accessible labels rather than claiming portal-tooltip coverage.

### 4. Utility Header & Search Trigger (`site-shell.tsx` & `learner-next-slice.css`)

- **Header Geometry**:
  - Desktop height is set to `76px` at `min-width: 1024px` to align with the sidebar brand header; mobile retains the existing safe-area-aware responsive height.
  - Background and border are transparent, with no backdrop blur.
...
- **Account Section**:
  ...
  - Profile uses the portal tooltip while collapsed and the native `title="Profile"` only while expanded, avoiding duplicate tooltip surfaces.
...
# Result: 31 test files passed, 465 tests passed (0 failed)
```
...
- **Release Reconciliation**:
  - The learner UI candidate was committed and pushed for review; no deployment had occurred when this evidence was captured.
  - Pre-existing local staging bridge and audit-document changes remained excluded and preserved in their original worktree.
- **Node 24 Enforcement**:
