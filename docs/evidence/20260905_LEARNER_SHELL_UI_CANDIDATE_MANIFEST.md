...
- Release target: the next exact-SHA staging release after PR #37 is merged and packaged.
- Deployment: none.
...
### Preserved but excluded local staging bridge repair

- `apps/learner-web/app/lib/dev-api-proxy.ts` and `.test.ts`
- `apps/admin-web/app/lib/dev-api-proxy.ts` and `.test.ts`
  - These dirty-worktree changes remain preserved in the persistent UI worktree.
  - They are not part of this candidate diff or the next staging release.

## Validation inventory

- Learner suite on the reconciled release branch: 31 files, 465 tests passed.
- Focused sidebar suite: 2 files, 39 tests passed.
...
| Learner login desktop                              | `http://learner.localhost:3000/login` | Stable light auth form; no offline fallback or reload loop                                                                             | Verified live in Chrome; durable exact-release capture remains pending | Passed locally |
| Learner login mobile                               | `http://learner.localhost:3000/login` | Responsive auth form with policy links and staging boundary                                                                            | Verified live in Chrome; durable exact-release capture remains pending | Passed locally |
...
- Do not claim exact-SHA staging evidence until the final PR #37 merge SHA is packaged, deployed, and independently verified live.
...
- Reconcile only the listed learner UI files into the release branch. Preserve the excluded bridge files, user-provided audit DOCX, and unrelated work in the persistent UI worktree.
