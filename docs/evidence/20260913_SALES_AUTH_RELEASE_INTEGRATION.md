...
Accepted e94c090545a51cb7b82cea5b50eb2a9215514213 adds the explicit Sales static-preview build before Python browser tests, with preview/telemetry environment confined to that single CI step. The production image build keeps its existing standalone setting. Exact static build and CI contract4tests passed; root reviewed workflow and guard diff. Existing control-plane workflow has no matching browser/static-export contract. Source AdminPeople correctionce5939d is integrated after real PostgreSQL full-file RED4pass1fail to GREEN5pass and independent review; exact protected rows are compared instead of assuming shared fixtures are globally empty.
