...
The final shell refinement keeps desktop navigation within the viewport while long reports scroll; the sidebar remains independently scrollable so account controls are reachable on short screens. Root also applied the repository formatter to the new workflow files. Sales Xray TypeScript passed locally (Node22.17.0; the immutable CI build uses pinned Node24). Browser verification of the viewport correction is assigned to the independent preview task and remains distinct from hosted report acceptance.

## Saved-call recovery release checks

The accepted-plan recovery and bounded owner-authorized continuation fixes are integrated through `7ed9888`. The source preserves original usage, source identity, budget reservations and audit history. A fresh owner plan can authorize one additional bounded processing window; malformed acceptance receipts and stale source bindings are rejected.

Root processing, backup and restore checks passed 1,171 tests with two environment-dependent skips (`lease-backup-root-7ed9888.xml`). Actual disposable PostgreSQL tests passed migration 0040 metadata parity and both continuation cases. The initial broader run passed 12 cases and failed one because this new checkout lacked its verified AudioAtlas binary. After restoring the source-bound ignored build artifact, that exact native integration case passed separately (`sales-lease-native-pg.junit.xml`); no test was skipped or weakened. Root Ruff format checks passed 636 files and mypy passed 286 source files. Receipts remain outside Git under `D:/AC-authority-closers-release-audit`.

Independent local browser evidence confirms the desktop sidebar stays within a 900px viewport and its account controls remain reachable while the report scrolls. These results establish local release readiness only; real staged report generation and production activation still require deployed evidence.

## Final execution-control integration

Application CI run 34839465488 failed at the Studio drag/drop test before image packaging: the asynchronous selection handler was still uploading when its success assertion ran. Commit 7ee49b1 waits for the confirmed processing state inside the existing act boundary; the focused upload file passed all eight cases. The failed run produced no deployable release image.

The reviewed execution-control leaf b7440e7 is integrated as 4c55add. The root also gates complete-plan acceptance before creating its acceptance receipt. Actual disposable PostgreSQL checks passed all six scenarios, including migration 0041 parity, paused acceptance preserving the quote and budget, resume, immutable control history, blocked queued work, retained report access, and another verified Admin pausing while a synthetic provider effect finishes. Receipt: `D:/AC-authority-closers-release-audit/sales-control-integrated-pg.junit.xml`; log SHA256 `ab01db4086c8bb55c2ca2c13174183f32a0977258b9fa39d782b98156528e1dd`.

Root focused processing/control/HTTP/rotation checks passed 45 tests. Ruff lint and format passed the canonical packages/python and tests scope (643 files); mypy passed 292 source files. New frontend files pass Prettier. Leaf evidence includes 1,227 unit/parity/restore tests, seven HTTP tests, five mounted UI tests, and both affected app type checks. No paid provider calls were made by these tests.

Pause does not cancel an already-started provider request. Existing commands by the same control identity can delay Admin admission on canonical Person/Session locks; another verified Admin and different guest owners do not share that contention. Budget exhaustion disables public admission, while direct quote requests retain their budget-conflict response. The shared money ledger remains authoritative across provider changes.

Source-owned staging credential rotation is integrated through a13b661. Its real hosted rotation and the final immutable CI build, staging report, production activation, and browser acceptance remain pending; none is claimed by local validation.

## Release-check repairs after 82cbe661

Application CI 34841623899 failed with 17 failures, 5,591 passes and 1,253 skips. Packaging did not run, so this source has no deployable core artifact. Its separately built web artifact is retained as superseded evidence and will not be paired with a different core release.

The integrated repairs update the migrated-table registry, current-consent error expectation, verification challenge clock, saved-provider-draft pinning contract, oversized-intake schema response, and the browser fixture's real execution-availability route. Disposable PostgreSQL checks passed the consent case, both verification cases and both authority cases. The standalone browser passed against actual PostgreSQL and TCP with the real availability route; its unexpected-network-failure assertions remain intact.

Synthetic rotation lifecycle tests now isolate the privileged host lock. The production directory guard also accepts the canonical root-owned sticky `/run/lock` ancestor while retaining stricter checks below it. Five synthetic lifecycle cases passed as Linux UID 1000 over SSH without a database connector, provider call or runtime change. Receipt: `D:/AC-authority-closers-release-audit/ci-34841623899-linux-nonroot-proof-20260914.json`, SHA256 `653e5d8165d72a4fac4e7942031c28372772c2e9138572fe1413591f05048052`. Root's combined registry and rotation checks passed 25 cases; Ruff lint and format passed 643 files. The real credential rotation remains pending.

The submission PostgreSQL file, rebuilt immutable artifacts, and live upload-to-report acceptance are still outstanding. The owner-approved trial allowance remains 3,600 seconds; stale tests must not revert that decision. Launch priority is a real source-bound report on standalone Sales Xray and Learner, followed by Admin cost/reviewer confirmation. Cheaper local transcription is a separate benchmark and is not an activated fallback.

Evidence correction: a copied diagnostic launcher reused the earlier `sales-control-integrated-pg.junit.xml` and log paths. Those paths no longer hold the original control run and must not be used to validate its historical hash. Root reran all six migration, pause, ledger and saved-report cases against a fresh disposable PostgreSQL database: six passed in 66.86 seconds. The replacement receipt is `D:/AC-authority-closers-release-audit/sales-control-repair-confirmation-pg.junit.xml`; the corresponding `sales-control-repair-confirmation-20260914.log` SHA256 is `8cc3ab96163755a78d195dd2fdd6ad7b58edef1d75c7f51a299ad3c13fb3a977`. No provider calls or hosted database changes occurred.

Submission repair `85766cc` is integrated as `f135d1f`: allowance responses and the exhausted-allowance fixture now match 60 minutes, and the newly supported learner host correctly rejects a cross-origin write with 403. No production allowance or host-guard code changed. All six tests in the submission file passed in 30.10 seconds against actual disposable PostgreSQL and the source-verified AudioAtlas executable. Receipt log: `D:/AC-authority-closers-release-audit/submission-ci-fix-full-20260914-0915.log`, SHA256 `03070d7231b72874f2371fb44db46ee5ffa8dc18dbabdf9f8cbc010cbaa1a4c0`. An exploratory native shim was removed before this proof and is not included in the release. Independent execution of the unchanged full-pipeline test also passed in 18.24 seconds; shared-schema queued work from earlier failed assertions is being checked as the cause of the original cascading worker failure.
