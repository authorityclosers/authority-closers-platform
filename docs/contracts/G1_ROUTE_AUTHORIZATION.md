...
| `GET /v1/admin/studio/readiness`                  | named owner/admin                     | `catalog_read`, active server-selected tenant; selected-tenant draft backlog only; no simulated operational rates         | AUTHZ-12    |
| `GET /v1/admin/studio/programs`                   | named owner/admin                     | `catalog_read`, active server-selected tenant; own programs plus immutable global read-only versions; bounded collection  | AUTHZ-12    |
| `GET /v1/admin/studio/programs/{id}`              | named owner/admin                     | `catalog_read`, active server-selected tenant, exact resource visibility; other-tenant/global-draft existence denied      | AUTHZ-12    |
| `POST /v1/admin/program-versions/{id}/publish`    | named owner/admin                     | `catalog_publish`, active tenant-owned draft, `If-Match`, idempotency ledger, reason, immutable transition, atomic audit   | AUTHZ-12    |
