...
|---|---|---|---|
| `POST /v1/auth/transactions` | anonymous | rate limit, safe return URL, server state/nonce | AUTHZ-01 |
| `GET /v1/auth/callback` | verified assertion | issuer, audience, state, nonce, one-time transaction, deterministic link | AUTHZ-02 |
