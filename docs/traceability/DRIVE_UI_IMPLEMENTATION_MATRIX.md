...
The candidate also adds login, forgot/reset password, offline, and canonical
`/session-expired` screens in the same visual family. These are required
contracts even though the reference folder does not contain a separate image
for every state.
