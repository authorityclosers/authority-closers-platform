...
- `db/migrations/versions/20260902_0013_planning_analytics.py` — existing
  Alembic chain migration.
- `apps/learner-web/app/calendar/` and `calendar-runtime.tsx` — explicit-plan
  Calendar surface using the existing learner shell.

Verification:

- `uv run ruff check packages/python/ac_platform db/migrations/versions/20260902_0013_planning_analytics.py tests/unit/http/test_planning_routes.py tests/database/test_planning.py`
