...
| Login and callback | `/login`, `/auth/callback`               | Controlled auth states; numbered UXS IDs pending | Provisional `AuthFlowPage`, `LoginForm`; `UI-C017`, `UI-C025`, `UI-C029`, `UI-C065`                    |
| Registration       | `/register`                              | Controlled auth states; numbered UXS IDs pending | Provisional `AuthFlowPage`, `RegistrationForm`; email/password only until OAuth consent binding exists |
| Verification       | `/verify-email`                          | Controlled auth states; numbered UXS IDs pending | Provisional `AuthFlowPage`, `VerificationForm`                                                         |
| Recovery and reset | `/forgot-password`, `/reset-password`    | Controlled auth states; numbered UXS IDs pending | Provisional `AuthFlowPage`, `RecoveryRequestForm`, `PasswordResetForm`                                 |
| Session recovery   | `/session-expired`                       | Controlled auth states; numbered UXS IDs pending | Provisional `AuthFlowPage`, `LoginForm`; explicit 401 reauthentication boundary                        |
