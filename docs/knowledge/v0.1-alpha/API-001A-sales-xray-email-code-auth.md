...
`POST /v1/auth/email-code/verify` accepts JSON fields `email`, `code`, `surface`, and `return_path`. It returns `authenticated`, `person_id`, `email`, `display_name`, `account_created`, `profile_complete`, and the validated `return_path`, and sets the host-only AC session cookie. It does not navigate the browser. `profile_complete` remains false until the dedicated required phone-profile completion check is integrated; WhatsApp profile data does not satisfy that check.
