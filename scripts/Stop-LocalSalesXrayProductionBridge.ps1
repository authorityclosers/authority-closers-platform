...
try {
    ...
    if (-not (Test-Path -LiteralPath $PidFile -PathType Leaf)) {
        Write-Host "No tracked local Sales Xray production bridge is running."
        return
    }
    ...
    Write-Host "Local Sales Xray production bridge stopped. Logs remain at $RuntimeDirectory."
}
finally {
